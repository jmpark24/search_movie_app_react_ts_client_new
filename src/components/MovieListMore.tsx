...
const MovieListMore: React.FC = () => {
  ...
  const { page, pageMax, searchText } = useSelector((state: RootState) => state.movie);
  const [showButton, setShowButton] = useState<boolean>(pageMax > page);

  useEffect(() => {
    setShowButton(pageMax > page);
  }, [page, pageMax]);

  const handleClick = async () => {
    dispatch(incrementPage());
    await dispatch(searchMovies({ page: page + 1, query: searchText }));
  };
